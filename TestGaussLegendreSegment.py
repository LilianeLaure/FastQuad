...
# regular function
def f(x,y):
	return exp(norm(x) + norm(y))#norm(x)**2 + norm(y)**2	#sin(norm(x) + norm(y))
	#return norm(x**2) + norm(y**2)
    
# singular function
def h(x,y):   
    return 1/(norm(norm(x) - norm(y)))
...
def calculateCompositeInt(int1, int2, N, order, t, w):
	...
	I = 0
	for i in range(0, numberIntervalls):
		for j in range(0, numberIntervalls):
			newInt1 = [np.array(a1 + i*(b1-a1)/numberIntervalls), np.array(a1 + (i+1)*(b1-a1)/numberIntervalls)]
			newInt2 = [np.array(a2 + j*(b2-a2)/numberIntervalls), np.array(a2 + (j+1)*(b2-a2)/numberIntervalls)]
...
def calculateError(int1, int2, N, order, t, w):
	...
	I = calculateCompositeInt(int1, int2, N, order, t, w)
	...
	I1 = calculateCompositeInt(int11, int21, N, order, t, w)
	#print "I1 =", I1
	I2 = calculateCompositeInt(int11, int22, N, order, t, w) 
	#print "I2 =", I2
	I3 = calculateCompositeInt(int12, int21, N, order, t, w)
	#print "I3 =", I3
	I4 = calculateCompositeInt(int12, int22, N, order, t, w)
	#print "I4 =", I4
	I_sub = I1 + I2 + I3 + I4
...
order = 5 # ordre for the method gaussLegendre
...
plt.title("regular function, order 5, N = 500")
